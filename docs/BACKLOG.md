...
| ~~31~~ | ~~**No custom date range on analytics**~~ | Step 16 | **CLOSED in Step 19** | Reports adds a from/to date picker (`GET /reports/sales`) over the same server-side aggregation Analytics uses — `AnalyticsService.overviewBetween`, so a report total is by construction the same number the dashboard shows. Dates are IST calendar-day bounds; CSV export is client-side from the server JSON (daily series only, no product names, so no CSV-injection surface). Verified in browser (30d ₹1,483.05 narrowed to ₹285.00 for today) and against the DB (148305 / 28500 paise match). Still IST-only (#30) and no per-restaurant timezone. |
