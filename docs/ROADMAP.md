...
**API endpoints:** `GET /api/v1/health`, `GET /api/v1/health/ready`, `POST /api/v1/auth/{register,login,refresh,logout,select-restaurant}`, `POST /api/v1/auth/{verify-email,resend-verification}`, `GET /api/v1/auth/me` (includes `emailVerified`), `POST|GET /api/v1/restaurants`, `GET|PATCH /api/v1/restaurants/current` (business profile: address/phone/GSTIN/FSSAI/receipt header+footer), `POST|GET /api/v1/products` (`?include=all` adds deactivated), `PATCH /api/v1/products/:id`, `POST|GET /api/v1/categories`, `PATCH|DELETE /api/v1/categories/:id`, `PUT /api/v1/categories/order`, `POST|GET /api/v1/orders` (create accepts `orderType`, `hold`, per-item `notes`; list accepts `cursor` keyset pagination), `GET /api/v1/orders/:id`, `GET /api/v1/orders/:id/timeline`, `PATCH /api/v1/orders/:id/status`, `POST /api/v1/orders/:id/payments` (split legs / pay-later; captured ≤ total), `POST /api/v1/orders/:id/refunds` (order.refund; reason required; ≤ captured; append-only `refunds` table), `POST|GET /api/v1/customers`, `GET /api/v1/customers/:id`, `GET /api/v1/customers/by-phone/:phone`, `PATCH /api/v1/customers/:id`, `POST|GET /api/v1/ingredients` (`?include=all` adds deactivated), `PATCH /api/v1/ingredients/:id`, `GET /api/v1/ingredients/:id`, `POST /api/v1/ingredients/:id/movements` (PURCHASE accepts supplierId + totalCostMinor), `POST /api/v1/ingredients/:id/adjustments`, `GET|POST /api/v1/suppliers`, `PATCH /api/v1/suppliers/:id`, `GET /api/v1/products/costing` (food cost + margin), `GET|PUT /api/v1/products/:id/recipe`, `GET|POST /api/v1/staff/invites`, `DELETE /api/v1/staff/invites/:id`, `GET|PATCH /api/v1/staff`, `POST /api/v1/staff/me/clock`, `POST /api/v1/staff/:id/clock`, `GET /api/v1/staff/timesheet`, `GET|POST /api/v1/join/:token`, `GET /api/v1/analytics/overview`, `GET /api/v1/ai/insights`, `GET|POST /api/v1/cash/sessions` (day-close: open till), `GET /api/v1/cash/sessions/current`, `GET /api/v1/cash/sessions/:id`, `POST /api/v1/cash/sessions/:id/movements` (cash in/out), `POST /api/v1/cash/sessions/:id/close` (count + settlement), `GET|POST /api/v1/marketing/coupons`, `PATCH /api/v1/marketing/coupons/:id`, `GET /api/v1/marketing/segments`, `GET /api/v1/marketing/segments/:key/customers`, `GET /api/v1/reports/{sales,gst,items,categories,settlement,voids,discounts}` (all take from/to IST days), `GET /api/v1/reports/audit` (audit.read; keyset `cursor`).
**Receipts:** GST bill (CGST/SGST split from per-line snapshots, 80mm print CSS) and KOT print from POS success / Orders / Kitchen via an in-page print area (`lib/receipt.tsx`); WhatsApp share via wa.me. CASHIER/KITCHEN hold `restaurant.read` so bills can carry the shop header.
**Web routes:** `/` (landing), `/login`, `/setup`, `/verify-email`, `/dashboard`, `/dashboard/menu`, `/dashboard/cash` (day close), `/dashboard/audit`, `/dashboard/pos`, `/dashboard/orders`, `/dashboard/customers`, `/dashboard/inventory`, `/dashboard/staff`, `/dashboard/kitchen`, `/dashboard/analytics`, `/dashboard/reports`, `/dashboard/ai`, `/dashboard/marketing`, `/join/[token]`.
...
Seeded: 4 roles (OWNER/MANAGER/CASHIER/KITCHEN), 24 permissions, 64 mappings.
Also: `refunds`, `cash_sessions`, `cash_movements`, `suppliers`, `email_verification_tokens`. Restaurant profile columns; `order_type` on orders; `email_verified_at` on users; `supplier_id`/`total_cost_minor` on stock_movements. Seeded: 25 permissions, 67 mappings.
